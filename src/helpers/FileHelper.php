...
class FileHelper
{
	 ...
	 * @throws Exception() if any of the exclude patterns is not a string or an array with keys: pattern, flags,
	 *     firstWildcard.
	 */
	 ...
	 *   A leading slash matches the beginning of the pathname. For example, `/*.php` matches `index.php` but not
	 *     `views/start/index.php`. An optional prefix `!` which negates the pattern; any matching file excluded by a
	 *     previous pattern will become included again. If a negated pattern matches, this will override lower
	 *     precedence patterns sources. Put a backslash (`\`) in front of the first `!` for patterns that begin with a
	 *     literal `!`, for example, `\!important!.txt`. Note, the '/' characters in a pattern matches both '/' and '\'
	 *     in the paths.
	 ...
	 *     to `true`.
	 * - `recursive`: boolean, whether the files under the subdirectories should also be looked for. Defaults to
	 *     `true`.
	 * @return array files found under the directory, in no particular order. Ordering depends on the files system
	 *     used.
